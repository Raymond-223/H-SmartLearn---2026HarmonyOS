...
def validate_profile_and_demo_contract() -> None:
    profile = (ETS_ROOT / "pages" / "LearnerProfilePage.ets").read_text(encoding="utf-8")
    demo = (ETS_ROOT / "pages" / "DemoCenterPage.ets").read_text(encoding="utf-8")
    for group, state in (
    ...
    require(demo, r"group:\s*'demo_profile'[\s\S]*?\.checked\(this\.selectedProfile\s*===\s*profile\.id\)", "demo profile selection")
    require(demo, r"group:\s*'demo_domain'[\s\S]*?\.checked\(this\.selectedDomain\s*===\s*id\)", "demo domain selection")
...
def main() -> int:
    ...
    validate_profile_and_demo_contract()
    ...
    print(json.dumps({
        ...
        "interaction_contracts": ["diagnostic", "resource_test", "practice", "profile", "demo"],
    }, ensure_ascii=False))
    return 0
