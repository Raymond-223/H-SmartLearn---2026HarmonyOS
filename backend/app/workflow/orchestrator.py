...
class Orchestrator:
    ...
    def _merge_agent_output(state: WorkflowState, agent_type: str, output: dict) -> None:
        if agent_type == "diagnosis_agent":
            ...
            state.retrieval_meta = {key: value for key, value in output.items() if key != "evidence_list"}
        elif agent_type == "risk_router_service":
            state.risk_route = output.get("risk_route")
        elif agent_type == "generation_agent":
            state.generated_resources = output.get("resources")
        elif agent_type == "proofgraph_service":
            state.claim_graph = output.get("claim_graph")
        elif agent_type == "review_agent":
            state.review_result = output
        elif agent_type == "critic_agent":
            state.critic_result = output
        elif agent_type == "judge_agent":
            state.judge_result = output
