...
The provider boundary is retained; when no LLM is configured, the same backend uses an evidence-backed deterministic generator.
...
class GenerationAgent(BaseAgent):
    ...
    def _apply_feedback_and_revision(
        ...
        instructions = []
        for item in agent_input.get("revision_instructions", []):
            if isinstance(item, dict):
                text = str(item.get("reason") or item.get("description") or item.get("action") or "").strip()
            else:
                text = str(item).strip()
            if text:
    ...
    async def _generate_with_provider(self, context: WorkflowState, agent_input: dict) -> AgentResult | None:
        ...
        try:
            ...
            metadata = {
                ...
                "risk_route": context.risk_route or {},
