...
class ReviewAgent(BaseAgent):
    ...
    def _review_evidence(self, resources: dict, evidence_list: list[dict]) -> list[dict]:
        ...
        def check_ids(ids: object, location: str, required: bool = True) -> None:
            ...
            for citation in ids:
                ...
                if verification_status == "verified":
                    pass
                elif verification_status == "trusted_source" and risk_level == "low":
                    pass
                elif verification_status == "trusted_source":
                    issues.append(self._issue(
                        layer, "evidence_requires_strict_verification",
                        f"evidence_list.{citation}",
                        f"证据 {citation} 属于{risk_level}风险，仅有trusted_source状态，需人工/规则/沙箱验证后才能用于高风险结论",
                        "high",
                    ))
                else:
        ...
        check_ids(guide.get("evidence_ids", []), "practice_guide.evidence_ids", required=False)
        for index, step in enumerate(guide.get("steps", []) if isinstance(guide.get("steps", []), list) else []):
            if isinstance(step, dict) and "evidence_ids" in step:
                check_ids(step.get("evidence_ids", []), f"practice_guide.steps.{index}.evidence_ids", required=False)
        ...
        for index, item in enumerate(graded.get("items", []) if isinstance(graded.get("items", []), list) else []):
            if isinstance(item, dict) and "evidence_ids" in item:
                check_ids(item.get("evidence_ids", []), f"graded_test.items.{index}.evidence_ids", required=False)
    ...
    async def run(self, context: WorkflowState, agent_input: dict) -> AgentResult:
        ...
        issues = structure_issues + evidence_issues + safety_issues
        ...
        return AgentResult(
            ...
            summary="三层审核与知识点JSON校验通过" if not issues else f"三层审核发现{len(issues)}个可定位问题",
        )
